...
def bereken():
    ...
    try:
        ...
        if aantal > 1 or aantal == 0:
            result.configure(text=str(aantal) + " frikandelbroodjes!")
        elif aantal<0:
            result.config(text="Lol poor")
        else:
            result.configure(text=str(aantal) + " frikandelbroodje!")
    except:
...
button = tk.Button(root, text="Bereken", command=bereken, bg="dark orange", activebackground="yellow", font=(font, fontsize_s, "bold"))
button.pack(pady=10)
